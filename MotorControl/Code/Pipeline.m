...
for i = 1:length(GroupNames)
    ...
    for j = 1:length(fieldnames(Participants))
        ...
        errorbar(barPositions + barWidth, mean3, std3, 'k.', 'LineWidth', 1);
...
for i = 1:length(GroupNames)
    ...
    for j = 1:length(fieldnames(Participants))
        ...
        TrainData{i, j} = mean(Train_errorData(1:end));
...
%         Test_errorData = myData.error_test;
        ...
        Baseline_errorData = mean(Baseline_errorData(end - (min_size - 1):end));
